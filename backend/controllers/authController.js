...
// Receptionist signup
const receptionistSignup = async (req, res) => {
  try {
    ...
    if (password.length < 6) {
     ...
     // Check for duplicates in the database
     const duplicateCheckQuery = "SELECT * FROM receptionists WHERE email = ? OR phone_number = ?";
     const [duplicates] = await db.query(duplicateCheckQuery, [email, phone_number]);
    ...
    const query = "INSERT INTO receptionists (full_name, email, phone_number, password, hospital_id) VALUES (?, ?, ?, ?, ?)";
    await db.query(query, [full_name, email, phone_number, hashedPassword, hospital_id]);

    res.status(201).json({ message: "Receptionist signed up successfully" });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Error signing up receptionist", error: err.message });
  }
};
...
const login = async (req, res) => {
  try {
    ...
    const userQuery = `
      ...
      UNION
      SELECT id, full_name, email, phone_number, password, 'receptionist' AS role FROM receptionists WHERE email = ? OR phone_number = ?;
    `;
